	...
	CommandLineRunner initNutritionData(NutrientLogRepository repository) {
		return args -> {
			...
			if (repository.count() == 0) {
				repository.saveAll(List.of(new NutrientLog(LocalDate.now().minusDays(6), 1850),
						...
						new NutrientLog(LocalDate.now().minusDays(1), 2150), new NutrientLog(LocalDate.now(), 1900)));
